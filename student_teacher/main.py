...
AUTH_PATH = ("/", "/index.html", "/user", "/problem",
             "/challenge", "/message", "/update-challenge", "/delete-challenge")
CHALL_PATH = "./chall"
...
def parse_user(data):
  field = ["userid", "username", "password",
           "fullname", "email", "phone", "is_teacher"]
...
def parse_answer(data):
  field = ["answerid", "userid", "username",
...
def form_data(request):
  if request.header.get("Content-Type", "") and request.header["Content-Type"] == "application/x-www-form-urlencoded" and request.rbody:
    # hacky
...
def homepage(request):
  ...
  if args["is_teacher"]:
    args["Other-User"] = [parse_user(u) for u in models["User"].get_users()
    ...
    if request.rbody.get("id", ""):
      if request.rbody["id"] == "new-user":
        if request.header["Current-User"]["is_teacher"]:
          ...
          if all((new_username, new_fullname, new_password, new_email, new_phone)):
            models["User"].insert_user(
                new_username, h1(new_password), new_fullname, new_email, new_phone)
            return request.redirect("/", is_post=True)
          else:
            args["error-msg-update"] = "User information field can not be empty"
        else:
          args["error-msg-update"] = "User don't have permission to add new user"
      else:
        userid = request.rbody["id"]
        if userid != str(request.header["Current-User"]["userid"]) and not request.header["Current-User"]["is_teacher"]:
          args["error-msg-update"] = "User don't have permission to change other user information"
        else:
          if not models["User"].get_user(userid):
            args["error-msg-update"] = "User don't exists"
          elif not request.header["Current-User"]["is_teacher"] and (request.rbody.get("username", "") or request.rbody.get("fullname", "")):
            args["error-msg-update"] = "User don't have permission to change this information"
          else:
            old_user = parse_user(models["User"].get_user(userid))
            new_username = request.rbody.get("username", "")
            new_username = new_username if new_username else old_user["username"]
            new_fullname = request.rbody.get("fullname", "")
            new_fullname = new_fullname if new_fullname else old_user["fullname"]
            new_email = request.rbody.get("email", "")
            ...
            result = models["User"].update_user(
                userid, new_username, new_password, new_fullname, new_email, new_phone)
            if result:
              return request.redirect("/", is_post=True)
...
def check_users(request):
  ...
  if request.header["Params"].get("id", ""):
    ...
    args["send-message"] = [parse_message(
        m) for m in models["Message"].get_send_message(request.header["Current-User"]["userid"], request.header["Params"]["id"])]
    ...
    args["user-info"] = [parse_user(u) for u in models["User"].get_users()
                         if u[0] != request.header["Current-User"]["userid"]]
      ...
      if request.rbody.get("deleteid", ""):
        ...
        if request.rbody["messageid"] == "new-message":
          ...
          if old_msg:
            ...
            args["error-msg"] = "Message not found"
      return request.redirect(f"/user?id={request.rbody['id']}&error-msg={args['error-msg']}", is_post=True)
...
def message_handle(request):
  ...
  for i in range(len(args["messages"])):
    args["messages"][i] = (args["messages"][i][0],
...
def do_problem(request):
  args = {}
  args["problem-list"] = [parse_problem(p)
                          for p in models["Problem"].get_problems()]
  args["is_admin"] = request.header["Current-User"]["is_teacher"]

  if args["is_admin"]:
    args["answer-list"] = [parse_answer(a)
                           for a in models["Answer"].get_answers()]
  else:
    args["answer-list"] = [parse_answer(a)
                           for a in models["Answer"].get_user_answers(request.header["Current-User"]["userid"])]
  ...
  if request.header["Params"].get("pid", ""):
    ...
    if a:
      a = parse_answer(a)
      if a["userid"] == request.header["Current-User"]["userid"] or (args["is_admin"]):
        ...
        if a:
          ...
          if a["userid"] == request.header["Current-User"]["userid"] or (args["is_admin"]):
            ...
            if not models["Problem"].get_problem(problemid):
              ...
              if models["Answer"].update_answer(a["answerid"], problemid, content):
                return request.redirect("/problem", is_post=True)
      ...
      if problemid:
        if problemid == "new-problem":
          ...
          if old_problem:
            ...
            problemname = request.rbody.get("problemname", "")
            problemname = problemname if problemname else old_problem["problemname"]
            content = request.rbody.get("problem-content", "").split(",", 1)
            content = content[1] if len(
                content) == 2 else old_problem["content"]
