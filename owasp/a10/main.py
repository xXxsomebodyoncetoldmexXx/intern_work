...
{% if avatar %}
  <img src="{{ avatar }}" width="640" height="480">
{% endif %}
...
def update_avatar():
  ...
  if url and uid:
    resp = get(url)
    if resp:
      img = f"data:{resp.headers['Content-Type']};base64, {b64encode(resp.content).decode()}"
      db.update_avatar(uid, img)
  return redirect(url_for("homepage", id=uid))
